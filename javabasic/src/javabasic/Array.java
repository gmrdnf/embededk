...
public class Array {
	public static void main(String[] args) {
		...
		// intd의 기본값이 0이므로 5개의 공간이 0으로 초기화 됨
		// float는 안에 0.0f 가들어감 boolean은 false가들어감
...
//		배열할당
		...
		// 문법적 오류가 없으므로 컴파일에는 문제가 없음
		// index범위는 0~4, 5인덱스는 범위 밖 => 실행 시에 ArrayIndexOutOfBoundsException
//		intArr[5]=6;

		//배열내의 요소들의 값을 출력
		//intArr.length는 5로 확정되어 있고 변경도 불가함
		//intArr.length는 연산은 1번이면 족한다. 
		//for문의 조건식에 사용하면 for문 반복회수만큼 연산하게 된다
		//마이크로소프트 :code complete
		//마틴 파울러 : refactoring
		int intArrLength = intArr.length;
	
		//그냥 아래 이for문만 쓰게되면 성능이 좋지않음
		//
		...
		//문자열 배열
		String[] strArr = {"hell","java","programming"};
