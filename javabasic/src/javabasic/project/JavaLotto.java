...
public class JavaLotto {
	...
	//1 =>6개를 입력했는지 확인하자.그렇지않다면 6개를 다시입력하도록하자
	//2 =>6개를 입력했는데 중복된 숫자가 있는지 확인하자.그렇지않다면 6개를 다시입력
	// 하도록하자
	//3 =>입력한 숫자의 범위가 1~45사이인지도 확인해야한다.그렇지않다면	
	// 그렇지 않다면 6개를 다시입력하도록하자
	//4 =>6개를 입력했는데 중복된 숫자가 있는지 확인하자
	// 그렇지 않다면 6개를 다시입력하도록하자
	//5 =>중복체크는 컴퓨터가 랜덤숫자 뽑아낼때도 사용하니 별도로 로직으로 분리하도록
	// 하자
	void inputUserNums() {
		Scanner scanner = new Scanner(System.in);
		String line = scanner.nextLine();
		...
		while (st.hasMoreTokens()) {
			...
			i++;//유저가입력한 번호를 다 userNums에 넣을때까지 반복한다
			//6개를 입력했는지 확인하자.그렇지않다면 6개를 다시입력하도록하자
			//이건 들어가기 전에 확인받아야하니 와일문 전에 if로 확인하고
			//아닐시에 다시 입력시켜야하니 else로 다시 돌리자
			//2번도 동일3번도 동일
